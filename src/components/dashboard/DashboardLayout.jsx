...
function DashboardLayout({ children }) {
  ...
  // Função para determinar se a rota atual está ativa
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <div className={`flex-1 ${isSidebarOpen ? "ml-64" : "ml-20"} transition-all duration-300`}>
        {/* Barra Superior */}
        <header
          className="bg-white/80 backdrop-blur-sm shadow-sm h-16 fixed right-0 left-0 z-20"
        ...
        {/* Área Principal */}
        <main className="pt-16 px-4 min-h-screen">{children}</main>
        ...
        <PublishProductCard isOpen={isPublishOpen} onClose={() => setIsPublishOpen(false)} />
      </div>
    </div>
  );
}
